...
def getNewSolution():
	newS = deepcopy(S)
	let ytotal be the number of CP of the closed facilities
	sort the facilities in the following way:
		- first the facilities of the zones whose gamma * d_z - current demand is highest
		- among those take the off-street facilities first
		for each of these facilities:
			fill it with CPs until gamma*d_z is met unless capacity or Nz is violated
	return newS.isFeasibleWithoutBudget(), newS




def isSwapFeasibleGlobally(S, facsToAdd, facsToRemove):



	# check capacity constraints
	# check zone demand
	# check zone on-street
	# check rapid
...
def globalLocalSearch():
	Op = get combinations of facilities to enter to the solution
	Cl = get combinations of facilities to remove from the solution
	for opcomb in Op:
		for clcomb in Cl:
			if swap(opcomb, clcomb) is globally feasible:
